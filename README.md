...
3. **Mark を作る**（中心ワークフロー）:
   ...
   - **note（必須）**を書く → **+ Add mark**。Marks リストに追加され、その瞬間のスクショも自動で撮られる。**mark は「いま選択中の time window」の範囲を凍結保持**するので（＝設定が mark の抽出範囲に効く）、バッファが流れても・リロードしても後から確実にコピーできる。`▸` で各 mark を展開すると、保持したイベント込みの中身をその場で確認できる。
   - → つまり `pick(任意) & note 入力 → Add mark → リストに追加`。
4. **タイムラインは常時記録され、抽出範囲（time window）は設定で決まる**:
   - 「直近 N 秒」/「チェックポイント以降」/**「このページだけ・直近 N ページ」**（ページ遷移境界で切る）/**「Clear timeline 以降」**（`⎚ Clear timeline` で“ここから”の cue を打つ。記録は消さない）。
   - この選択が **mark の抽出範囲にも、窓コピーにも同時に効く**。
5. **コピーは3通り**:
   - **Mark 単体**: Marks リスト各項目の **Copy**。
   - **Mark リスト全体**: ヘッダの **Copy all**（全 mark を 1 束に）。
   - **時間窓**: **Copy window context**。
   - 不要な mark は各項目の **✕** で、まとめて消すならヘッダの **Clear** で削除。
6. クリップボードに事実だけのテキスト束が入り、スクショは `Downloads/trawler/` に保存され本文にはパスのみが載る。
7. Claude Code に貼って「これを保存して直して」と指示する。
